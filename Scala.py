...
executable_path = r"C:\Users\vvasilev005\Desktop\Scala.exe"
...
def schedule_reboot(hour):
    ...
    while True:
        ...
        if now.weekday() == 1 and now.strftime("%H:%M") == hour:  # modification for the day
            reboot()
...
def reverse_counter(reboot_time, reboot_weekday):
    ...
    if current_time >= target_datetime:
        print(f"The {reboot_weekday.capitalize()} at {reboot_time} has already passed.")
        return
