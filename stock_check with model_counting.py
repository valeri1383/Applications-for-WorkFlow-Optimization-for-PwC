...
# Function to count how many PCs per model in the 'Model Category' column
def count_pcs_per_model(model_category):
    return Counter(model_category)
...
def update_result_label():
    ...
    result_text.insert(tk.END, f"EP Stock Check ({current_datetime})\n\n", "header")
...
def export_to_csv():
    ...
    if export_file_path:
        with open(export_file_path, mode='w', newline='') as file:
            ...
            current_datetime = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
            writer.writerow([f"EP Stock Check ({current_datetime})"])
            writer.writerow(
            ...
            for item in matching_items:
                for row in list_stock:
                    if row[0].upper() == item:
                        writer.writerow(row)
            writer.writerow([])
            writer.writerow([f"Total accounted assets: {len(matching_items)}"])

            # Write missing assets
            writer.writerow(["Missing Assets"])
            writer.writerows([[item] for item in missing_assets])
            writer.writerow([])
            writer.writerow([f"Total missing assets: {len(missing_assets)}"])

            # Write extra items
            writer.writerow(["Extra Items"])
            writer.writerows([[item] for item in extra_in_scanned])
            writer.writerow([])
            writer.writerow([f"Total extra assets: {len(extra_in_scanned)}"])

            # Count models in 'Model Category'
            model_category_list = [row[1] for row in list_stock if row[1]]
            model_counts = count_pcs_per_model(model_category_list)
            writer.writerow(["Model Category Counts"])
...
# Function to show model counts in the result panel
def show_model_counts():
    ...
    current_datetime = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    result_text.insert(tk.END, f"Model Counts ({current_datetime})\n\n", "header")

    for model, count in model_counts.items():
        result_text.insert(tk.END, f"{model}: {count}\n")
...
# Add a button to show model counts
model_counts_button = tk.Button(button_frame, text="Show Model Counts", fg='blue', bg='lightgrey', command=show_model_counts)
model_counts_button.pack(side=tk.LEFT, padx=5)
